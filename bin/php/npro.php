<?php
    ...
    if(strcasecmp($id,"") == 0 || strcasecmp($name,"") == 0 || strcasecmp($grade,"") == 0)
        ...
        newPro($id, $name, $kate, $skate, $grade, $qawl, $strm, $scut, $svac, $ssaw, $spkg, $smp, $sfrz, $hsell, $kates, $gol, $katej);
        ...
        for($i = 0; $i < count($lgdg); $i++){
            $qty = 0;
            if(strcasecmp($lgdg[$i][0], $dsett[3][3]) == 0)
                $qty = $qawl;

            newGdgPro($lgdg[$i][0], $id, $qty, $db);
        }
    }
